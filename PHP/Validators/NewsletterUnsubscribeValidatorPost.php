...
class NewsletterUnsubscribeValidatorPost extends Validator
{
    /* Properties */
    ...
    private $newsUnsubRegex = '/(?>[^\p{C}\p{M}\p{N}\p{P}\p{S}\p{Z}0-9;\' %-])/u';
    ...
    public function __construct(Validator $formTokenValidator, Validator $emailValidator, array $filteredInput, array $transitoryInputs = NULL)
    {
        $phpFVA = [
                    ...
                    'newsUnsubBtn' => ['filter'  => FILTER_VALIDATE_REGEXP,
                                       'flags'   => FILTER_REQUIRE_SCALAR,
                                       'options' => ['regexp' => $this->newsUnsubRegex]]
                  ];

        $phpFEMA = [
                        'ajaxFlag'     => 'Illegal AJAX flag!',
                        'nocache'      => 'Illegal nocache value!',
                        'newsUnsubBtn' => 'Invalid input for button element!'
                   ];
        ...
        $validationMA = [
                            ...
                            'newsUnsubBtn' => ['kind' => 'word', 'type' => 'string', 'min' => 11, 'max' => 11, 'pattern' => '/(?>\A(?:Unsubscribe){1}?\z){1}?/u', 'noEmptyString' => true, 'specificValue' => 'Unsubscribe', 'rangeOfValues' => NULL]
                        ];
    ...
    protected function translateValidatedInput()  //Translate form inputs into database data.
    {
        ...
        $this->translatedInputArray['newsUnsubBtn'] = NULL;
        unset($this->translatedInputArray['newsUnsubBtn']);
        return;
    }
